...
public class GameMode2 {
	public void playGameMode2() throws InterruptedException {
		...
		if(num==1) {pb.equipFighterBack(p1,false);}
		else {pb.equipFighterBack(p1,true);}
		num=0;
		System.out.println();

		System.out.println("プレイヤー2のキャラメイクです。");
		System.out.println();
		Fighter p2=pb.chooseCharacter();
		pb.putName(p2);
		pb.upperPara(p2);
		pb.downerPara(p2);
		System.out.println("武器を選択しますか？1:yes 2:no↓");
		num=checkNumber.checkAndReturnNumber(1,2);
		if(num==1) {pb.equipFighterBack(p2,false);}
		else {pb.equipFighterBack(p2,true);}
		num=0;
		System.out.println();
